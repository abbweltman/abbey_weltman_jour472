...
diverse_CSA_high <- Racial_Diversity_Index_1_ %>%
  filter(CSA2010 == c("North Baltimore/Guilford/Homeland", "Mount Washington/Coldspring", "Harbor East/Little Italy", "South Baltimore", "Canton")) %>%
   select(racdiv21,CSA2020) %>%
   arrange(desc(racdiv21)) 

diverse_CSA_low <- Racial_Diversity_Index_1_ %>%
  filter(CSA2010 == c("Clifton-Berea", "Cherry Hill", "Southwest Baltimore", "Greater Rosemont", "Sandtown-Winchester/Harlem Park")) %>%
   select(racdiv21,CSA2020) %>%
   arrange(desc(racdiv21)) 
