...
```{r}
#Now I was to plot these findings on a map. first I am going to make a dataset. That we can apply to the map with the parameters of the regions. First I am going to inter join the GOID with the new_business_high_data. 

balt_tract_shapefiles <- read_rds("baltimore_tract_shapefiles.rds")

CSA2020 <- Census_Tract_2020 %>% 
...
#Spatial Join
new_business_high_geo <- balt_tract_shapefiles %>% 
  st_join(new_business_high_sf)

ggplot() +
  geom_sf(data=balt_tract_shapefiles) +
  geom_sf(data=new_business_high_sf) 
  geom_sf_label(biz4_21)
```
