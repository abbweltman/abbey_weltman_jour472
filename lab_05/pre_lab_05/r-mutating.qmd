...
```{r}
city_md_income <- read_csv("~/Documents/GitHub/abbey_weltman_jour472/lab_05/pre_lab_05/city_md_income.csv")
View(city_md_income)
...
```{r}
Beaver Creek CDP had the highest absolute change of 118292
Shaft has the highest percent change of 399.88%
```
...
#the value for the first quartile is $63327
```
```{r}
...
#At the end of the code I would add slice max and a slice min tool to format the tables and then add a head or n code so it only selects the top 10 vlaues. 
