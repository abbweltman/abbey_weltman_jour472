...
```{r}
maryland_tracts2 <- read_excel("maryland_tracts.xls")
...
md_counties_geoids
 ...
 md_smith_data <- md_counties_geoids %>% 
  inner_join(maryland_tracts2, by=c("GEOID"="cty"))
...
```{r}
#If I attempt to inner_join the two dataframes based on the county name as the common key, the resulting dataframe will only contain rows where there are matching county names in both dataframes. For example, if there is a county called "Kent County" in both dataframes, the join will successfully match the population and large employer count for Kent County. However, if there are discrepancies in the county names between the two dataframes like if one dataframe lists "Kent County, Maryland" and the other lists "Kent County" these rows will not be matched in the inner join, resulting in missing data. The inner join will retain only the counties that have both population information and large employer count available in both dataframes. You could fix this problem by cleaning the data or seperating the county names from the state 
